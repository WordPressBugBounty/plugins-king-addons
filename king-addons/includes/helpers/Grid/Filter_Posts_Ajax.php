...
HTML;
    ...
    public function render_post_thumbnail($settings) {
        ...
        if (!empty($settings['grid_lazy_loading']) && $settings['grid_lazy_loading'] === 'yes') {
            echo '<img data-no-lazy="1" src="'.esc_url(KING_ADDONS_URL.'includes/assets/libraries/grid/icon-loading.png').'" '
            ...
            if ($sec_hover === 'yes') {
                echo '<img data-no-lazy="1" src="'.esc_url($src2).'" alt="'.esc_attr($alt).'" '
                    .'class="king-addons-hidden-img king-addons-animation-timing-'
                    .esc_attr($settings['image_effects_animation_timing']).'">';
            }
        } else {
            echo '<img data-no-lazy="1" src="'.esc_url($src).'" alt="'.esc_attr($alt).'" '
