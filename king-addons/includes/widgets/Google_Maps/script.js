...
(function ($) {
    $(window).on("elementor/frontend/init", () => {
        elementorFrontend.hooks.addAction(
            "frontend/element_ready/king-addons-google-maps.default",
            ($scope) => {
                elementorFrontend.elementsHandler.addHandler(
                    elementorModules.frontend.handlers.Base.extend({
                        onInit() {
                            ...
                            const controls = googleMap.data("controls");
                            const locations = googleMap.data("locations") || [];
                            const gMarkers = [];
                            ...
                            locations.forEach((loc) => {
                                const marker = createMarker(loc, map);
                                if (!marker) return;
                                gMarkers.push(marker);
                                bounds.extend(marker.position);
                                // Create info window if needed
                                if (loc.gm_show_info_window !== "none") {
                                    createInfoWindow(marker, loc, map);
                                }
                            });
                            ...
                            if (settings.cluster_markers === "yes") {
    ...
    /**
     * Creates and returns a Google Maps marker, or null if latitude/longitude is invalid.
     */
    function createMarker(data, map) {
        if (!data.gm_latitude || !data.gm_longitude) return null;

        const iconSizeW = data.gm_marker_icon_size_width.size;
        const iconSizeH = data.gm_marker_icon_size_height.size;
        const iconOptions =
            data.gm_custom_marker === "yes"
                ? {
        ...
        return new google.maps.Marker({
            map,
            position: {
                lat: parseFloat(data.gm_latitude),
                lng: parseFloat(data.gm_longitude),
            },
    ...
    /**
     * Creates and binds an InfoWindow to a marker based on location data.
     */
    function createInfoWindow(marker, data, map) {
        const content = `
      <div class="king-addons-gm-iwindow">
        <h3>${data.gm_location_title}</h3>
        <p>${data.gm_location_description}</p>
      </div>`;
        ...
        if (data.gm_show_info_window === "load") {
            iWindow.open(map, marker);
        } else {
            marker.addListener("click", () => iWindow.open(map, marker));
        }
    }

    /**
     * Returns a parsed style array based on the settings.style option.
     * Falls back to an empty array if no match is found.
     */
    function getMapStyle(settings) {
        const styleMap = {
            simple:
        ...
        if (settings.style === "custom") {
            return JSON.parse(settings.custom_style || "[]");
        }
        const rawStyle = styleMap[settings.style] || "[]";
